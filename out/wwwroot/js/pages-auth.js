...
function submitPhoneNumber() {
  ...
  // Format phone number for Lebanese numbers
  // Remove any spaces, dashes or other non-digit characters (except +)
  phoneNumber = phoneNumber.replace(/[^\d+]/g, '');

  // Handle various Lebanese mobile number formats
  if (phoneNumber.match(/^(\+?961|0)?(3|7[0-9])\d{6}$/)) {
    // It's a Lebanese mobile number in various formats, standardize it
    ...
    // Generic handling for other phone formats
    ...
    if (!phoneNumber.startsWith('+')) {
      ...
      if (phoneNumber.startsWith('961')) {
        ...
        // Short number likely without country code
        phoneNumber = '+961' + phoneNumber;
      } else {
        // Just add + for international format
        phoneNumber = '+' + phoneNumber;
      }
    }
  }
